...
const OurProducts = () => {
  return (
    <div className="flex flex-col justify-center items-center mt-10 w-full px-4">
      ...
      <h2 data-aos="fade-up"
                data-aos-delay="200" className="text-2xl font-bold mb-4 text-center md:hidden">
        Our Products
      </h2>
          ...
          !item.isCenter && ( // Skip the "Our Products" text item
            <div key={index} className="relative w-20 h-20 sm:w-24 sm:h-24">
              <img data-aos="fade-up"
                data-aos-delay={item.aosDelay} src={item.src} className="w-full h-full" alt={item.title}  />
              <div className="absolute inset-0 flex items-center justify-center">
                <p data-aos="zoom-in"
                data-aos-delay={item.delayTitle} className="text-sm sm:text-xs md:text-sm lg:text-base text-white font-bold">
                  {item.title}
                </p>
              </div>
            </div>
          ...
          >
            {column.map((item, itemIndex) =>
              item.isCenter ? (
                ...
                >
                  <p data-aos="fade-up"
                data-aos-delay={item.aosDelay} className="text-xl sm:text-lg md:text-2xl text-black text-center font-bold">
                    {item.text}
                  </p>
                </div>
              ) : (
